...
const sidebarItems: SidebarItem[] = [
  ...
  },
  {
    ...
    // ลบ permission ออก - ให้ admin ทุกคนเข้าถึงได้
  },
  {
    title: "จัดการแผนก",
  ...
  },
  {
    ...
    permission: "manage_admins" // เก็บ permission ไว้ - เฉพาะ SuperAdmin เท่านั้น
  },
...
const Sidebar = () => {
  ...
  // ตรวจสอบสิทธิ์ผ่าน API - แก้ไขเพื่อให้ admin ทั่วไปเข้าถึงได้มากขึ้น
  const checkUserPermission = (permission: string): boolean => {
    if (!userData) return false
    
    // SuperAdmin เข้าถึงได้ทุกอย่าง
    if (userData.isSuperAdmin) return true
    
    // สำหรับ permission พิเศษเท่านั้น (เช่น manage_admins)
    if (permission === "manage_admins") {
      return userData.permissions?.includes(permission) || false
    }
    
    // Admin ทั่วไปเข้าถึงได้ทุกอย่างยกเว้น manage_admins
    return true
  }
  ...
  // Handle route protection - ปรับให้จำกัดเฉพาะหน้าจัดการ admin เท่านั้น
  useEffect(() => {
    if (!userData || isLoading) return

    const protectedRoutes = [
      { path: "/backend/admin-manage", permission: "manage_admins" },
      // ลบ admin-patient-form ออกเพื่อให้ admin ทั่วไปเข้าถึงได้
    ]
        ...
        <nav className="mt-2 px-3">
          <div className="space-y-1">
            ...
            {sidebarItems.map((item, index) => {
              // ตรวจสอบสิทธิ์แบบปลอดภัย - ปรับใหม่
