...
const AdminPage = () => {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "ไม่สามารถโหลดข้อมูลแผนกได้ โปรดลองอีกครั้ง",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleAddDepartment = async () => {
    if (!newDepartmentName.trim()) {
      toast({
        title: "กรุณาระบุชื่อแผนก",
        description: "ชื่อแผนกห้ามเป็นค่าว่าง",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "สำเร็จ!",
        description: `เพิ่มแผนก "${newDepartmentName}" เรียบร้อยแล้ว`,
      });
      fetchDepartments();
    } catch (error) {
      toast({
        title: "เกิดข้อผิดพลาด",
        description: error instanceof Error ? error.message : "การเพิ่มแผนกล้มเหลว",
        variant: "destructive"
      });
    } finally {
  ...
  const handleDeleteDepartment = async () => {
    ...
    try {
      ...
      toast({
        title: "สำเร็จ!",
        description: `ลบแผนก "${deletingName}" เรียบร้อยแล้ว`,
      });
      fetchDepartments();
    } catch (error) {
      toast({
        title: "เกิดข้อผิดพลาด",
        description: error instanceof Error ? error.message : "การลบแผนกล้มเหลว",
        variant: "destructive"
      });
    } finally {
      setActionLoading(false);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      toast({
        title: "สำเร็จ!",
        description: `แก้ไขชื่อแผนกเป็น "${editedName}" เรียบร้อยแล้ว`,
      });
      fetchDepartments();
    } catch (error) {
      toast({
        title: "เกิดข้อผิดพลาด",
        description: error instanceof Error ? error.message : "การแก้ไขแผนกล้มเหลว",
        variant: "destructive"
      });
    } finally {
      setActionLoading(false);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <Card className="md:col-span-5 bg-white shadow-lg rounded-lg border-none">
          ...
          <CardFooter className="flex justify-end">
            <Button onClick={() => setOpenAddModal(true)} className="w-full">
      ...
      <Dialog open={openEditModal} onOpenChange={setOpenEditModal}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              กรุณากรอกชื่อแผนกใหม่ที่ต้องการแก้ไข
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <label htmlFor="new-name" className="text-sm font-medium">ชื่อแผนกใหม่:</label>
              <Input 
                id="new-name"
                value={editedName} 
      ...
      <Dialog open={openDeleteModal} onOpenChange={setOpenDeleteModal}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              การลบแผนกไม่สามารถเรียกคืนได้ กรุณาตรวจสอบให้แน่ใจก่อนดำเนินการ
            </DialogDescription>
          </DialogHeader>
